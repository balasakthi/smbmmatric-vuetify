<template>
  <PageLayout>
    <template v-slot:pageHero>
      <PageHero :pageHero="pageHero.management" />
    </template>

    <template v-slot:pageHeader>
      <PageHeader :pageHeader="pageHeader.management" />
    </template>
    ...
    <template v-slot:pageContent>
      <v-container class="mt-8 pa-6 pa-md-12">
        ...
        <v-row v-else>
          <v-col
            v-for="management in managementData"
            :key="management.name"
            cols="12"
            sm="6"
            lg="4"
          >
            <v-avatar
              v-if="management.image_url"
              class="rounded-xl border"
              :image="management.image_url"
              :size="256"
              rounded="0"
            />
            <v-icon
            ...
            <v-list-item
              class="px-0 mt-2"
              :subtitle="management.name || 'Unknown Name'"
              :title="management.title || 'No Title Provided'"
            />
          </v-col>
        </v-row>
      </v-container>
...
async function fetchManagement() {
  try {
    ...
    if (Array.isArray(response?.data)) {
      managementData.value = response.data.filter(
